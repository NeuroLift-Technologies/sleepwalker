...
### Continuity Contract Checks

When changing `sleepwalker_protocol/continuity.py`,
`sleepwalker_protocol/protocol.py`, `src/continuity.ts`, or `src/protocol.ts`,
verify these source-level contracts in docs and tests:

- Continuity is keyed by a stable user identifier (`user_id` in Python,
  `userId` in TypeScript), never by message text.
- Assessment reads continuity context (`continuity_context` /
  `continuityContext`) but does not implicitly write session state.
- Explicit persistence goes through `maintain_continuity(user_id, data)` or
  `maintainContinuity(userId, data)`.
- Per-user JSON files stay inside `storage_path` / `storagePath` and use the
  slug plus SHA-256 filename generated by `ContinuityManager`; do not derive or
  document raw `${userId}.json` filenames.
